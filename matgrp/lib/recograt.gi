...
solvNC,S,pcgs,x,r,c,w,a,bound,U,xp,depths,oldsz,prime,relord,gens,acter,ogens,stabs,n,strongs,stronglevs,laynums,slvec,layerzero,p,laynum,layers,sel,vals,stronglayers,layervecs,slpval,slp,baspts,levp,blocksz,lstrongs,lstrongsinv,bl,opt,check,primes,shortlim,orb,orbs,j,sz;
  ...
  local oldstrong,oldsz,bas,pbp;
    oldsz:=Size(S);
    oldstrong:=ShallowCopy(StrongGenerators(S));
    # work around the ommission of prior redundant base points
    bas:=BaseStabilizerChain(S!.opt.Base);
    pbp:=First([1..Length(bas.points)],x->bas.ops[x](bas.points[x],g)<>bas.points[x]);
    if not bas.points[pbp] in BaseStabilizerChain(S).points then
      ...
      AddGeneratorToStabilizerChain(S,g);
    fi;
    while Size(S)=oldsz do 
      Info(InfoFFMat,2,"orderr BUG !!!\n");
      AddGeneratorToStabilizerChain(S,g);
    od;
...
local r,m,f,a,p,i,homs,hom,img,ff,ffp,ffpi,ffppc,ffhoms,ffsubs,d,elmimg,
  ...
  fac,idmats,bas,basrep,s,triv,addPcElement,procrels,addCleanUpper,k,l;
